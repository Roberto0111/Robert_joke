...
LaunchAgent com.roberto.robert-joke
  ...
  -> 依每日受控成長實驗製作 14-18 秒 2.5D 動態沉穩風格直式 Reel
...
使用既有內容只測動態影片，不生成、不 commit、也不發 IG：

```bash
/usr/bin/python3 scripts/run_daily_pipeline.py --preview-reel-from 2026-08-16_2030
```

預覽會輸出至 `previews/<run-id>_motion_preview.mp4`。每日 Reel 使用 `cinematic_2_5d_v1`：首幀直接顯示漫畫，背景與漫畫頁以不同速度緩慢移動，第一頁輕推，第二頁結論加強推近。
...
固定發布 Reel，兩張四格原稿使用 2.5D 緩推與呼吸位移，搭配原創沉穩背景音；每日只測一組開場、題材、結論、CTA 與 14/16/18 秒閱讀節奏
```
